...
public class AnalysisInput {
	...
	public void singleOracle () 
	{
		File input = new File("input//log_with_oracle_3days.csv");
		File output = new File("output//log_with_single_oracle.csv"); 
		...
		try {
			...
			while (fileScanner.hasNext() ) {
				...
				for(int i = 0; i< Debug.ORACLE_MAX_ARRAY_INDEX; i++) {
					totalResults += Integer.parseInt(afterSplit[5+i]);
				}
				//System.out.println(afterSplit[0] +"\t"+afterSplit[1]+"\t"+afterSplit[2]+"\t"+afterSplit[3]+"\t"+afterSplit[4]+"\t"+ totalResults);
				Files.append(afterSplit[0] +"\t"+afterSplit[1]+"\t"+afterSplit[2]+"\t"+afterSplit[3]+"\t"+afterSplit[4]+"\t"+ totalResults+"\n", output, Charsets.UTF_8);
			}
	...
	public void getHistogramWithLocalSimTime() {
		...
		try {
			...
			while (fileScanner.hasNext() && counter < 100000) {
				String inputStrig = fileScanner.nextLine();
				String[] afterSplit = inputStrig.split(",");
				int entryTime = Integer
						.parseInt(afterSplit[afterSplit.length - 2]);
				int cacheTime = Integer
						.parseInt(afterSplit[afterSplit.length - 1]);
				int timeDifferent = 0;
				int unit = 1000;
				if (entryTime == cacheTime) {
					timeDifferent = Integer
							.parseInt(afterSplit[afterSplit.length - 3])
							- Integer
									.parseInt(afterSplit[afterSplit.length - 4]);
					;
					unit = 100;
